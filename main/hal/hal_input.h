...
/// Logical roles. Physical mapping confirmed on hardware:
///   GPIO10 = UPPER side button -> kNextCard
///   GPIO9  = LOWER side button -> kRepeat
///   GPIO1  = TOP button, in the middle of the title bar -> kLetterInWord
/// The fourth button (lowest on the side) is PWR_KEY, wired to the PMIC as
/// the power/wake key, and is not readable as a GPIO.
enum class Button : uint8_t { kNextCard = 0, kRepeat = 1, kLetterInWord = 2, kCount = 3 };
